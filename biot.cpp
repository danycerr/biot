...
void biot_problem::assembly(double dt) {
	...
	workspace.add_expression("[0,-0].Test_u +[+0.0].Test_p*tau + (0/bm)*p_old.Test_p + alpha*Test_p*Div_u_old", mim);
...
void biot_problem::assembly_p(double dt){
	...
	// Pressure equation
	workspace.add_expression("(0/bm + beta)*p.Test_p + tau*permeability*Grad_p.Grad_Test_p", mim); // tau
	...
	// tau
	workspace.add_expression("[+0.0]*Test_p*tau + (0/bm)*p_old.Test_p + alpha*Test_p*Trace(Sym(Grad_u_old))", mim); // tau
...
void biot_problem::assembly_u(double dt){
	...
	workspace.clear_expressions();
	workspace.add_expression("[0,-0].Test_u  + alpha*p*Div_Test_u ", mim);
	workspace.assembly(1);
	workspace.clear_expressions();
  ...
  while(fix_count < max_iter && ( rel_unorm>epsu ||  rel_pnorm > epsp) )
		{
         ...
         // gmm::clear(P);
         {
          ...
          iter.set_noisy(1);               // output of iterations (2: sub-iteration)
          iter.set_maxiter(1000); // maximum number of iterations
          gmm::gmres(Kp, P, Bp, PM, restart, iter);
         ...
         gmm::diagonal_precond<sparse_matrix_type> PRu(Ku);
         {
          ...
          // gmm::clear(U);
          gmm::gmres(Ku, U, Bu, PM, restart, iter);
        }
         //--------------------------------------------------------------
         #ifdef PRINT_MATRIX
